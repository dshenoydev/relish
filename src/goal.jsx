...
const GoalConfigContext = React.createContext(null);
...
function GoalNav() {
  const C = React.useContext(GoalConfigContext);
  return (
    ...
    <section id="reading" className="goal-articles">
      <div className="shell">
        <div className="section-head">
          ...
          <a href="/" className="btn btn-ghost btn-arrow">All articles on Relish</a>
        </div>
      ...
      <div className="shell">
        <div className="footer-top" style={{ gridTemplateColumns: "1.6fr 1fr 1fr" }}>
          ...
          <div className="footer-col">
            ...
            <ul>
              <li><a href="/lean">Lean Out & Feel Light</a></li>
              <li><a href="/strength">Build Strength From the Inside</a></li>
              <li><a href="/longevity">Optimum Health · the long game</a></li>
            </ul>
          </div>
          <div className="footer-col">
...
export default function GoalApp({ config }) {
  useEffect(() => {
    if (config && config.accent) applyGoalTheme(config.accent);
  }, [config]);

  if (!config) return <div style={{ padding: 40 }}>No goal config provided.</div>;

  return (
    <GoalConfigContext.Provider value={config}>
      <div className={"goal-page goal-" + config.slug}>
        <GoalNav />
        <main>
          <GoalHero />
          <GoalSignup />
