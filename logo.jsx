...
//
// Why a pomegranate?
//   · Iconic whole-food shape (round body + crown — instantly recognizable)
//   · Culturally tied to Diya (Indian founder) — used in Ayurveda, sweets, festivals
//   · Nutritionally signature — antioxidants, "superfruit" without the marketing
//   · Rarely used in nutrition branding (most reach for leaves & apples)
//   · Works monochrome, in a circle frame, mirrored, cross-sectioned, or as a single seed
//
// Locked palette:
//   Forest #1F3D2E  (the crown)
//   Terracotta #C25A38  (the body)
//   Amber #B27D2B  (the seeds / highlight)
//   Cream #F4EDDD  (the flesh / negative space)
...
function MarkPom({ s = 1 }) {
  ...
  return (
    <svg viewBox="0 0 44 44" width={px} height={px} aria-hidden="true">
      ...
      {/* Highlight — implies sheen / dimensionality */}
      <ellipse cx="16" cy="22" rx="3.6" ry="5" fill={POM_COLORS.amber} opacity="0.35" />
    </svg>
  );
}

/* ── 2. Open — cross-section, seeds showing ────────────────────────── */
...
function MarkSeed({ s = 1 }) {
  ...
  return (
    <svg viewBox="0 0 40 40" width={px} height={px} aria-hidden="true">
      {/* Aril — teardrop using an ellipse with a small tail */}
...
/* ── 5. Crown — minimal monogram mark ──────────────────────────────── */
function MarkCrown({ s = 1 }) {
  const px = 40 * s;
  return (
    <svg viewBox="0 0 40 40" width={px} height={px} aria-hidden="true">
      {/* Big crown — same shape, oversized, no body */}
      ...
      {/* Single dot under — implies the body without drawing it */}
      <circle cx="20" cy="32" r="3" fill={POM_COLORS.terracotta} />
    </svg>
  );
}

/* ── 6. Badge — pom inside a circle frame (for profile pics) ───────── */
...
const MARK_VARIANTS = {
  ...
  crown: MarkCrown,
  badge: MarkBadge,
  // Back-compat aliases — old keys still resolve to pom
  wedge: MarkPom,
  pip: MarkCluster,
  orb: MarkPom,
  slice: MarkOpen,
...
function RelishLogo({ variant = "pom", size = 1, color = "var(--forest)", showWord = true }) {
...
// "Stamp" version — for footer / large display. Mark and word vertically stacked.
function RelishStamp({ variant = "pom", size = 1, color = "var(--cream-soft)" }) {
  const Mark = MARK_VARIANTS[variant] || MarkPom;
  return (
...
window.RelishLogo = RelishLogo;
window.RelishStamp = RelishStamp;
window.LOGO_VARIANTS = ["pom", "open", "seed", "cluster", "crown", "badge"];
